...
async function loadUserStats() {
    try {
        ...
        if (response.success) {
            ...
            // Update stat cards
            document.getElementById('userStatActive').textContent = stats.active_borrowings || 0;
            document.getElementById('userStatReturned').textContent = stats.returned_borrowings || 0;
            document.getElementById('userStatTotal').textContent = stats.total_borrowings || 0;
...
async function startQRScanner() {
    try {
        ...
        try {
            ...
            showToast('Gagal mengakses kamera. Pastikan browser memiliki izin kamera.', 'error');
        }
    }
}
...
function displayCart() {
    ...
    cartContainer.innerHTML = borrowCart.map((item, index) => `
        <div class="cart-item">
    ...
    container.innerHTML = borrowings.map(borrowing => `
        <div class="borrowing-card" onclick="showBorrowingDetail(${borrowing.id})">
            <div class="borrowing-header">
                <div>
                    <div class="borrowing-date">
                        <i class="fas fa-calendar"></i> Dipinjam: ${formatDate(borrowing.borrow_date)}
                    </div>
                    ${borrowing.expected_return_date ? `
                        <div class="borrowing-date" style="font-size: 12px; margin-top: 3px;">
                            <i class="fas fa-clock"></i> Rencana kembali: ${formatDateSimple(borrowing.expected_return_date)}
                        </div>
                    ` : ''}
                    ${borrowing.actual_return_date ? `
                        <div class="borrowing-date" style="font-size: 12px; margin-top: 3px; color: var(--success-color);">
                            <i class="fas fa-check"></i> Dikembalikan: ${formatDate(borrowing.actual_return_date)}
                        </div>
                    ` : ''}
                </div>
                ${getStatusBadge(borrowing.status)}
            ...
            <div class="borrowing-items">
                ...
                ${borrowing.items ? borrowing.items.slice(0, 3).map(item => `
...
async function showReturnModal(borrowingId) {
    try {
        ...
        if (response.success) {
            ...
            if (itemsList) {
                itemsList.innerHTML = borrowing.items.map(item => `
    ...
    if (returnForm) {
        returnForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const response = await apiRequest(API_ENDPOINTS.BORROWINGS.RETURN(borrowingId), {
                    method: 'PUT',
                    body: formData
                });
