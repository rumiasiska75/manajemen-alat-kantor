...
// @access  Private
router.post('/', authenticateToken, upload.single('photo_evidence'), async (req, res) => {
  try {
    ...
    const borrowingItems = typeof items === 'string' ? JSON.parse(items) : items;
    ...
    const photoPath = req.file ? `/uploads/evidence/${req.file.filename}` : null;
...
// @access  Private
router.put('/:id/return', authenticateToken, upload.single('photo_evidence'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    for (let item of borrowingItems) {
      ...
      const returnInfo = returnItems.find(ri => ri.tool_id === item.tool_id);
      ...
      if (conditionAfter !== 'baik') {
        await database.run(
          'UPDATE tools SET condition = ? WHERE id = ?',
          [conditionAfter, item.tool_id]
        );
      }
    }

    // Update borrowing status
    const photoPath = req.file ? `/uploads/evidence/${req.file.filename}` : borrowing.photo_evidence;
...
router.get('/stats/summary', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    const totalResult = await database.get('SELECT COUNT(*) as count FROM borrowings');
    stats.total = totalResult.count;

    const activeResult = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'active\'');
    stats.active = activeResult.count;

    const returnedResult = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'returned\'');
    stats.returned = returnedResult.count;

    const pendingResult = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'pending\'');
    stats.pending = pendingResult.count;

    const cancelledResult = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'cancelled\'');
