...
app.get('/api/dashboard/stats', require('./middleware/auth').authenticateToken, require('./middleware/auth').isAdmin, async (req, res) => {
  try {
    const stats = {};

    // Total users
    const usersResult = await database.get('SELECT COUNT(*) as count FROM users');
    stats.total_users = usersResult.count;

    // Total tools
    const toolsResult = await database.get('SELECT COUNT(*) as count FROM tools');
    stats.total_tools = toolsResult.count;

    // Available tools
    const availableResult = await database.get('SELECT COUNT(*) as count FROM tools WHERE available_quantity > 0');
    stats.available_tools = availableResult.count;

    // Borrowed tools
    const borrowedResult = await database.get('SELECT COUNT(*) as count FROM tools WHERE available_quantity < quantity');
    stats.borrowed_tools = borrowedResult.count;

    // Active borrowings
    const activeBorrowings = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'active\'');
    stats.active_borrowings = activeBorrowings.count;

    // Pending borrowings
    const pendingBorrowings = await database.get('SELECT COUNT(*) as count FROM borrowings WHERE status = \'pending\'');
...
app.get('/api/dashboard/user-stats', require('./middleware/auth').authenticateToken, async (req, res) => {
  try {
    const stats = {};
